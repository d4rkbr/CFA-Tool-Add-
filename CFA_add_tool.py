import os, sys, re, PIL.Image , tkinter, tkinter.filedialog, tkinter.messagebox, tkinter.scrolledtext, 
from tkinter import *  
#Globals
dir = ""
image = ""
...
def main() :   
	...
	def clickImage():
		global image
		image = tkinter.filedialog.askopenfilename(title = "Select image",filetypes = (("png","*.png*"),("jpeg","*.jpg")))
		imgTextVar.set(image)
		
	def clickSubmit():
		if (not dir) :
			tkinter.messagebox.showinfo("Error", "Please set directory")
		elif (not image) :
			tkinter.messagebox.showinfo("Error", "Please set image")
		elif (not nameEntryVar.get()) :
			tkinter.messagebox.showinfo("Error", "Name cannot be empty")
		elif (not powerEntryVar.get()) :
			tkinter.messagebox.showinfo("Error", "Power cannot be empty")
		elif (not clanOptionVar.get()) :
			tkinter.messagebox.showinfo("Error", "Please set clan")
		else :
			submit()
	...
	tkinter.Label(window, text = "Shield").grid(row = 5, column = 5, sticky=E)
	global shieldEntryVar
	shieldEntryVar = tkinter.StringVar()
	tkinter.Entry(window, textvariable = shieldEntryVar, width=10).grid(row = 5, column = 7, sticky=W+E)
	
	tkinter.Label(window, text = "Grade").grid(row = 6, column = 5, sticky=E)
	global gradeOptionVar
	gradeOptionVar = tkinter.StringVar()
	gradeOptions = tkinter.OptionMenu(window, gradeOptionVar, "", *"")
	gradeOptions.grid(row = 6, column = 7, sticky=W+E)
	...
	tkinter.Label(window, text = "Clan").grid(row = 7, column = 5, sticky=E)
	global clanOptionVar
	clanOptionVar = tkinter.StringVar()
	global clanOptions
	clanOptions = tkinter.OptionMenu(window, clanOptionVar, "", *"")
	clanOptions.configure(state="disabled")
	clanOptions.grid(row = 7, column = 7, sticky=W+E)
	
	tkinter.Label(window, text = "Race").grid(row = 8, column = 5, sticky=E)
	global raceEntryVar
	raceEntryVar = tkinter.StringVar()
	tkinter.Entry(window, textvariable = raceEntryVar).grid(row = 8, column = 7, sticky=W+E)
	
	tkinter.Label(window, text = "Type").grid(row = 9, column = 5, sticky=E)
	global typeOptionVar
	typeOptionVar = tkinter.StringVar()
	typeOptions = tkinter.OptionMenu(window, typeOptionVar, "", *"")
	typeOptions.grid(row = 9, column = 7, sticky=W+E)
	...
	tkinter.Label(window, text = "Gift").grid(row = 10, column = 5, sticky=E)
	global giftOptionVar
	giftOptionVar = tkinter.StringVar()
	giftOptions = tkinter.OptionMenu(window, giftOptionVar, "", *"")
	giftOptions.grid(row = 10, column = 7, sticky=W+E)
...
def getCard(cardNum) :
	...
	shield = "global.DefensePowerStat[CardStat] = %s" % shieldEntryVar.get()
